...
import { children, guardians, authorizations, exitLogs } from "../drizzle/schema";
...
try {
  ...
  for (let i = 0; i < 150; i++) {
    ...
    const age = Math.floor(Math.random() * 4) + 2;
    ...
    childrenData.push({
      ...
      faceEmbedding: generateFakeEmbedding() as any,
  ...
  await db.insert(guardians).values(guardiansData);
  console.log(`✅ ${guardiansData.length} responsáveis criados`);

  // 3. Criar autorizações
  ...
  const childIds = (childrenRows as any[]).map(row => row.id);
  const guardianIds = (guardiansRows as any[]).map(row => row.id);

  for (const childId of childIds) {
    const numGuardians = Math.floor(Math.random() * 2) + 2;
    const selectedGuardians = new Set<number>();
  ...
  // 4. Criar 5 dias de registros de saída
  ...
  for (let day = 0; day < 5; day++) {
    ...
    for (let i = 0; i < childrenPerDay; i++) {
      ...
      if ((authRows as any[]).length > 0) {
        const randomAuth = (authRows as any[])[Math.floor(Math.random() * (authRows as any[]).length)];
    ...
    console.log(`  ✅ Dia ${day + 1}: ${childrenPerDay * 2} registros`);
  }
...
📊 Resumo dos dados criados:
  ...
  - ${authorizationsData.length} autorizações
